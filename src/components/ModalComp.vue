...
const { actualModal } = storeToRefs(appStore)

const _componentName = computed(() => actualModal.value.component)

const _componentToRender = shallowRef()

onMounted(() => {
	if (_componentName.value) {
		_componentToRender.value = defineAsyncComponent(() =>
			import(`../components/${_componentName.value}.vue`)
		)
	}
})

watch(_componentName, (componentName) => {
	if (componentName) {
		_componentToRender.value = defineAsyncComponent(() =>
			...
			<div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
				...
				>
					...
					<button
						...
						@click.prevent.stop="appStore.closeModal"
					>
						<svg
							class="w-3 h-3"
							aria-hidden="true"
